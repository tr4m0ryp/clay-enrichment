...
def _extract_contact_fields(contact: dict) -> dict:
    """Extract denormalized contact fields from a contact row."""
    return {
...
def _extract_company_fields(company: dict) -> dict:
    """Extract denormalized company fields from a company row."""
    return {
...
async def _fetch_company_map(
    ...
    """Pre-fetch company data for all contacts, keyed by company ID string."""
    needed: set[str] = set()
    for c in contacts:
...
async def _process_pair(
    ...
    # Read company body for enrichment context
    company_summary = ""
    if company_id:
        from src.db.companies import CompaniesDB as _CB
        company_summary = await contacts_db._pool.fetchval(
    ...
    if contact_id not in updated_contacts:
        ...
        if determined_role and determined_role != cf["job_title"]:
            update_kwargs["job_title"] = determined_role
        context_summary = sr.get("context_summary", "")
        if context_summary:
            update_kwargs["context"] = context_summary
        if update_kwargs:
            await contacts_db.update_contact(contact_id, **update_kwargs)
...
async def campaign_scoring_worker(
    ...
    while True:
        try:
            ...
            # Query contacts with status = 'Researched'
            ...
            for contact in contacts:
                cid = str(contact.get("company_id", "")) if contact.get("company_id") else ""
